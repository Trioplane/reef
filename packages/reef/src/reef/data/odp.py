...
def create_reef_odp_data_namespace(ctx: Context, opts: ReefPluginOptions):
    class ReefSpecialOdpData(File):
        ...
        def generate_odp_resources(
            ...
            logger.debug("TEXT FONT SIZES: %s", text_properties)
            logger.debug("----")
            logger.debug("Text '%s' positioned %s with font size of %s", text_contents[0], text_positions[0], text_properties[0]["fo:font-size"])
            logger.debug("------------------------------------------------------")
            
    return ReefSpecialOdpData
...
def odp(ctx: Context, opts: ReefPluginOptions):
    ...
    ctx.data.extend_namespace.append(create_reef_odp_data_namespace(ctx, opts))