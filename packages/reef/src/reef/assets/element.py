...
def create_reef_element_asset_namespace(ctx: Context, opts: ReefPluginOptions):
    class ReefElementAsset(JsonFileBase):
        ...
        def generate_graphic_element(
            ...
            pack[namespace].item_models[path] = ItemModel({
                ...
                **({"tints": [{ "type": "minecraft:constant", "value": opts.tint }]} if opts.tint is not None else {})
            })
...
def element(ctx: Context, opts: ReefPluginOptions):
    ...
    ctx.assets.extend_namespace.append(create_reef_element_asset_namespace(ctx, opts))